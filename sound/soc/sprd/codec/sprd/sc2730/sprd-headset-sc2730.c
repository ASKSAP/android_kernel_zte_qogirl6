...
/* ZTE_MODIFY */
#if defined(CONFIG_SPRD_FSA4480_I2C)
...
#if defined(CONFIG_SPRD_FSA4480_I2C)
	if (pdata->fsa_handle) {
		pr_info("fsa4480 mic/ground switch");
		if (high)
			fsa4480_switch_event(pdata->fsa_handle, FSA_SET_MIC_TO_SBU1);
		else
			fsa4480_switch_event(pdata->fsa_handle, FSA_SET_MIC_TO_SBU2);
	}
#else
...
#if defined(CONFIG_SPRD_FSA4480_I2C)
	if (pdata->fsa_handle) {
		pr_info("fsa4480 audio attached(%d)", hdst->typec_attached);
		fsa4480_switch_event(pdata->fsa_handle,
			(hdst->typec_attached ? FSA_USBC_AUDIO_ATTACHED : FSA_USBC_AUDIO_UNATTACHED));
	}
#endif
...
#if defined(CONFIG_SPRD_FSA4480_I2C)
	pdata->fsa_handle = of_parse_phandle(np, "fsa4480-i2c-handle", 0);
	pr_info("property fsa4480-i2c-handle%sfound in node %s\n", (!pdata->fsa_handle ? " not " : " "), np->full_name);
#endif
