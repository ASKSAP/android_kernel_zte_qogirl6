...
int panel_set_aod_brightness(u32 level)
{
	...
	if (level == 0) {
		data_level_l[1] = 0x30;
		pr_info("AOD: Panel set RM692C9 aod brightness to level 0x30\n");
		...
		data_level_m[1] = 0x5c;
		pr_info("AOD: Panel set RM692C9 aod brightness to level 0x5C\n");
		for (i = 0; i < sizeof(reg_addr); i++) {
		...
		data_level_h[1] = 0xb0;
		pr_info("AOD: Panel set RM692C9 aod brightness to max level: 0xb0\n");
		for (i = 0; i < sizeof(reg_addr); i++) {
