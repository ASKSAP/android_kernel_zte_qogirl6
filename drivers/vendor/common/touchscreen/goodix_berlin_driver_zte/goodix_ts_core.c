...
static int goodix_ts_power_init(struct goodix_ts_core *core_data)
{
	...
	dev = core_data->bus->dev;
	ts_bdata = board_data(core_data);

	if (strlen(ts_bdata->avdd_name)) {
		ts_info("Avdd name is not NULL");
	...
	if (strlen(ts_bdata->iovdd_name)) {
		ts_info("iovdd name is not NULL");
...
{
	struct goodix_bus_interface *bus_interface = NULL;
	int ret = 0;
	...
	if(!pdev) {
		ts_err("Invalid platform_device");
	...
	ret = ts_core_data->hw_ops->reset(ts_core_data, GOODIX_NORMAL_RESET_DELAY_MS);
	if (ret) {
		ts_err("reset fail");
		goto err_out;
	} else {
	...
	if (ret) {
		...
		ts_info("goodix device confirm success");
	}
	//return -EINVAL;/////////////////////////////////////////////////////////////OK
