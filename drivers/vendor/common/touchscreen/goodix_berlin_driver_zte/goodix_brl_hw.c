...
{
	struct goodix_ts_board_data *ts_bdata = board_data(cd);
	int r = 0;

	if (on) {
		ts_info("brl_power_on");
		/* must guarantee iovdd enbaled before avdd */
		if (cd->avdd) {
			r = regulator_enable(cd->avdd);
			if (!r) {
				ts_info("regulator enable SUCCESS11");
		...
		if (cd->iovdd) {
			r = regulator_enable(cd->iovdd);
			if (r)
				ts_err("Failed to enable iovdd power:%d", r);
				return r;
		} else {
			ts_info("success to enable iovdd power");
		}

		/*usleep_range(15000, 15100);
		ts_info("gpio_direction_output rst 1");
		gpio_direction_output(cd->board_data.reset_gpio, 1);
		r = brl_reset_after(cd);
		if (r < 0) {
			...
			ts_info("reset_after process success");
		}*/
		msleep(GOODIX_NORMAL_RESET_DELAY_MS);
		return 0;
	}
	/*power off process */
	ts_info("brl_power_off");
	if (cd->avdd) {
		r = regulator_disable(cd->avdd);
		if (!r) {
