...
static int gtp_cfg_bin_upgrade(struct goodix_ts_core *core_data, const struct ts_firmware *gtp_firmware)
{
	...
	ret = goodix_get_reg_and_cfg(core_data, cfg_bin->cfg_pkgs->cnst_info.sensor_id, cfg_bin);
	...
	ret = ts_dev->hw_ops->send_config(core_data, cfg_bin->bin_data, cfg_bin->bin_data_len);
	//brl_send_config(struct goodix_ts_core *cd, u8 *cfg, int len)
	if (ret < 0) {
