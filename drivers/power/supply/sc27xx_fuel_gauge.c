...
#define SC27XX_FGU_BOOT_CAPACITY_THRESHOLD	200
#define SC27XX_FGU_OCV_CAPACITY_ZERO		3450000
...
static int sc27xx_fgu_get_boot_capacity(struct sc27xx_fgu_data *data, int *cap)
{
	...
	*cap *= 10;
	...
	if (!is_first_poweron) {
		...
		if (ocv_now <= SC27XX_FGU_OCV_CAPACITY_ZERO) {
			SPRD_FGU_DEBUG("ocv too low,  recaculate cap at boot: %d\n", ocv_cap_now);
			*cap = ocv_cap_now;
		} else if ((abs(pocv_cap * 10 - saved_cap) < SC27XX_FGU_BOOT_CAPACITY_THRESHOLD) ||
		    (pocv >= SC27XX_FGU_DENSE_CAPACITY_LOW &&
		     pocv <= SC27XX_FGU_DENSE_CAPACITY_HIGH)) {
		     *cap = saved_cap;
		}
