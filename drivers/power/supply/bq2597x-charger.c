...
static int bq2597x_update_bits(struct bq2597x_charger_info *bq, u8 reg,
				    u8 mask, u8 data)
{
	int ret;
	u8 tmp;
...
static int bq2597x_parse_dt(struct bq2597x_charger_info *bq, struct device *dev)
{
	...
	bq->cfg->regulation_disable = of_property_read_bool(np,
			"ti,bq2597x,regulation-disable");
...
static void bq2597x_check_fault_status(struct bq2597x_charger_info *bq)
{
	...
	dev_err(bq->dev, "%d %d %d %d %d %d %d %d\n", bq->bat_ovp_fault,
			bq->bat_ocp_fault,
			bq->bus_ovp_fault,
			bq->bus_ocp_fault,
			bq->bat_therm_fault,
					...
					const struct i2c_device_id *id)
{
	...
	dev_err(bq->dev, "bq2597x probe successfully, Part Num:%d\n!",
				bq->part_no);
